...
// sharedCachePrefix — placed at the TOP of both plannerPrompt and reviewerPrompt so the
// large task+tokens block is identical in both calls. This maximises the chance of an
// ephemeral-5m prompt-cache hit between the PLAN and REVIEW calls within the same run.
// MUST remain byte-for-byte identical across callers — do not add role-specific text here.
function sharedCachePrefix(task, tokens) {
  return `# Orchestra Run Context\n## Task\n${task}\n\n## Design System Tokens\n${tok(tokens)}`;
}
...
export function plannerPrompt(task, tokens, ctx, consiliumOn) {
  ...
  const consiliumNote = consiliumOn
    ...
    : "";
  // shared prefix first (cache-warming: identical block at the top of plan + review calls)
  const PREFIX = sharedCachePrefix(task, tokens);
  return `${PREFIX}

# Planner Instructions
You are the DESIGNER + ORCHESTRATOR in a multi-agent build pipeline.
A SEPARATE executor agent WRITES all the code. You do NOT write or edit files.
Your job: turn the Task above into a precise, UI-aware implementation spec the executor can follow exactly.
...
2. If the task touches UI, fill "design": name each component, its role, and the EXACT token names it must use
   (from the Design System Tokens above). Never specify a raw hex color or raw dp number when a token covers it.
   Enumerate render "states" (default, loading, empty, error, pressed/focused/disabled where applicable),
   "interactions", "a11y" (>=48dp Android / >=44px web·iOS hit-targets, accessible names, contrast >=4.5:1),
   and "motion". Set "copy_needed":true if new user-facing strings are required.
...
export function reviewerPrompt(task, spec, diff, verifyOut, tokens, ctx, imageManifestMd, refNote) {
  ...
  // shared prefix first (cache-warming: identical block at the top of plan + review calls within same run)
  const PREFIX = sharedCachePrefix(task, tokens);
  return `${PREFIX}

# Reviewer Instructions
You are the REVIEWER. The executor just produced the diff below. Review STRICTLY.
...
/* Iteration 2+ — diff-aware retry message with reviewer feedback.
 *
 * Diff-aware design: sends ONLY the targeted delta (feedback + unmet criteria + violations),
 * NOT the full spec. The executor already has the spec from iter 1; restating it dilutes focus.
 * changedFiles (optional string[]) tells the executor what it already touched — helps it avoid
 * starting from scratch while fixing only what the reviewer flagged.
 * trapsBlock (optional string from ctx.sections.executorTraps) injects known recurring failure
 * patterns for this repo so the executor avoids repeating them.
 */
export function executorRetryMessage(spec, lastFinalVerdict, ctxSlice, { changedFiles, trapsBlock } = {}) {
  const v = lastFinalVerdict || {};
  const lines = [];

  // Primary feedback block (most important — put first for attention)
  ...
  // Design system violations (blocking only for retry focus)
  const dsv = Array.isArray(v.design_system_violations) ? v.design_system_violations : [];
  const dsvBlocking = dsv.filter((d) => d && d.severity === "blocking");
  for (const d of dsvBlocking) {
    const where = d.file ? `${d.file}${d.line != null ? ":" + d.line : ""}` : "";
    const want = d.expected ? ` → use ${d.expected}` : "";
    lines.push(`- [DESIGN/${where || "?"}] ${d.issue || ""}${want}`);
  }
  // Non-blocking violations (advisory, deduped)
  const dsvOther = dsv.filter((d) => d && d.severity !== "blocking").slice(0, 3);
  for (const d of dsvOther) {
    const where = d.file ? `${d.file}${d.line != null ? ":" + d.line : ""}` : "";
    const want = d.expected ? ` → use ${d.expected}` : "";
    lines.push(`- [DESIGN-WARN/${where || "?"}] ${d.issue || ""}${want}`);
  }

  // A11y findings (blocking only)
  const a11y = Array.isArray(v.a11y_findings) ? v.a11y_findings : [];
  for (const a of a11y.filter((a) => a && a.severity === "blocking")) {
    const ref = a.wcag ? `/${a.wcag}` : "";
    const fix = a.fix ? ` (fix: ${a.fix})` : "";
    lines.push(`- [A11Y${ref}] ${a.issue || ""}${fix}`);
  }

  // Unmet acceptance criteria (explicit checklist)
  const crit = Array.isArray(v.criteria_check) ? v.criteria_check : [];
  const unmet = crit.filter((c) => c && c.met === false);
  if (unmet.length) {
    lines.push("\nStill-unmet acceptance criteria (address ALL of these):");
    for (const c of unmet) lines.push(`- [UNMET] ${c.criterion || ""}${c.note ? " — " + c.note : ""}`);
  }

  // Constraints reminder (brief — executor already knows the full spec)
  const s = spec || {};
  ...
  // Files touched in the failing attempt (diff-aware hint)
  if (Array.isArray(changedFiles) && changedFiles.length) {
    lines.push(`\nFiles you already touched (build on them, don't rewrite from scratch):\n${changedFiles.map((f) => `- ${f}`).join("\n")}`);
  }

  // Known failure patterns for this repo
  const traps = typeof trapsBlock === "string" ? trapsBlock.trim() : "";
  if (traps && !/^\(no known/i.test(traps)) {
    lines.push(`\nKnown failure patterns for this executor in this repo (avoid repeating):\n${traps}`);
  }
  ...
  return `Previous attempt was REJECTED (score=${v.score ?? "?"}). Apply ONLY the targeted fixes below, then stop.
Do NOT rewrite unrelated code — make surgical changes to the files that need fixing.
